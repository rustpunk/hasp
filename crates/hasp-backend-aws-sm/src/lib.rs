...
///
/// **Note:** Explicit proxy configuration is not yet supported for AWS SDK
/// backends. Use the `HTTPS_PROXY`/`HTTP_PROXY` environment variables instead.
#[derive(Debug)]
...
impl AwsSmBackend {
    ...
    /// Create a new `AwsSmBackend` with an explicit proxy.
    ///
